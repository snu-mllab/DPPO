...
def get_queries_from_multi(env, dataset, num_query, len_query, data_dir=None, balance=False, label_type=0, skip_flag=0):
    ...
    return batch
def append_dones_float(dataset):
...
def find_timestep_in_traj(dones_float, start):
    for i in range(start, 0, -1):
        if dones_float[i-1] == 1:
            return start - i
    return start


def load_queries_with_indices(dataset, len_query, saved_indices, saved_labels, scripted_teacher=False):

    dataset = append_dones_float(dataset)

    len_saved_labels = len(saved_labels)
    len_dataset = len(dataset["rewards"])
    ...
    for k, v in total_d.items():
        v["rewards"] = np.zeros((len_saved_labels, len_query))
        v["observations"] = np.zeros((len_saved_labels, len_query, observation_dim))
        v["actions"] = np.zeros((len_saved_labels, len_query, action_dim))
        v["timestep"] = np.zeros((len_saved_labels, len_query), dtype=np.int32)
        v["mask"] = np.zeros((len_saved_labels, len_query), dtype=np.int32)
    
    for i in tqdm(range(len_saved_labels), desc="get queries from saved indices"):
        for j in [1, 2]:
            start = saved_indices[j-1][i]
            end = min(start + len_query, len_dataset-1) # -1 is for masking below
                
            total_d[j]["rewards"][i, :end-start] = dataset["rewards"][start:end]
            total_d[j]["observations"][i, :end-start] = dataset["observations"][start:end]
            total_d[j]["actions"][i, :end-start] = dataset["actions"][start:end]
            timestep_start = 1
            timestep_end = timestep_start + end - start
            total_d[j]["timestep"][i, :end-start] = np.arange(timestep_start, timestep_end)
            total_d[j]["mask"][i, :end-start][:] = 1

    batch = {}
    if scripted_teacher: 
        human_labels = np.zeros((len_saved_labels, 2))
        sum_rewards_1 = total_d[1]["rewards"].sum(axis=1)
        sum_rewards_2 = total_d[2]["rewards"].sum(axis=1)
        binary_label = 1*(sum_rewards_1 < sum_rewards_2)
        rational_labels = np.zeros((len(binary_label), 2))
        rational_labels[np.arange(binary_label.size), binary_label] = 1.0
        batch["labels"] = rational_labels
    else:
