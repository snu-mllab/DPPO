...
def visualize_query(
    ...
    for seg_idx in trange(num_query):
        ...
        for t in trange(query_len, leave=False):
            gym_env.sim.set_state(dataset["observations"][start_indices[t]][:-1])
            gym_env.sim.forward()
        ...
        gym_env.reset()
        for t in trange(query_len, leave=False):
            gym_env.sim.set_state(dataset["observations"][start_indices_2[t]][:-1])
            gym_env.sim.forward()
...
def main(_):
    ...
    human_indices_2_file, human_indices_1_file = sorted(os.listdir(base_path))
