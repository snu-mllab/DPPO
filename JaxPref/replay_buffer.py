...
def get_ood_batch(trajs, starts, query_len=100):
    batch = {}
    assert len(trajs) % 2 == 0
    for key in trajs[0].keys():
        data = []
        for traj, start in zip(trajs, starts):
            data.append(traj[key][start:start+query_len])
        batch[key] = np.stack(data, axis=0)

    data = [(np.arange(1, 1 + query_len)) for _ in range(len(trajs))]
    batch["timestep"] = np.stack(data, axis=0)
